...
const recruiters = 
{
    joe : { 'username' : 'joe', userType : 'recruiter', postings: [
        {
          ...
          },
          {
            ...
            ratings: 10,
            openings: 5,
            applicants: ['kishore','swaroop','jorts'], 
          },
          {
            ...
            ratings: 8,
            openings: 450, 
            applicants: ['swaroop', 'jorts']
          }

    ] },
    andy : {'username' : 'andy', userType : 'recruiter', postings: [
        {
    ...
    katie: { 'username' : 'katie', userType : 'recruiter', postings : [{
...
function isNewUser(username){
    return !recruiters.hasOwnProperty(username.toLowerCase())
}

function createRecruiterData(username) {
    recruiters[username] = { username, userType: 'recruiter', postings: []}
}

function addToRecruiterPostings({username, posting}) {
   const  updatedPosting = {...posting, applicants: []}
    recruiters[username]['postings'].push(updatedPosting)
    
}

function addApplicants ({username, postingId , postedBy}) {
    const posting = recruiters[postedBy]['postings'].find(post => post.postingId === postingId)
console.log(posting)
    if (!posting['applicants'].includes(username)) {
        posting['applicants'].push(username);
    }
...
function getRecruiterPostings(username) {
    return recruiters[username]['postings']

}

function removeApplicants ({username, postingId , postedBy}) {
  const posting = recruiters[postedBy]['postings'].find(post => post.postingId === postingId)
console.log(posting)
posting['applicants'] = posting['applicants'].filter(name => name !== username)

}
module.exports = {
    isValid,
    getUserData,
    addToRecruiterPostings,
    getRecruiters,
    isNewUser,
    createRecruiterData,
